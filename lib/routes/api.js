...
router.get('/merkle/root/:key', (request, response) => {
  ...
  KeyStoreHistory.findOne({key})
    ...
    .catch(error => {
      logger.error(`GET /merle/root/:key {key: ${key}} ${error.message || error}`);
...
router.post('/merkle/verify', (request, response) => {
  ...
  KeyStoreHistory.findOne({key})
    ...
    .catch(error => {
      logger.error(`POST /merle/verify {key: ${key}, merkleRoot: ${merkleRoot}} ${error.message || error}`);
      return errorResponse(response, error.message || error);
    })
})
...
router.post('/merkle/contains', (request, response) => {
  ...
  KeyStoreHistory.findOne({key}).populate('history').exec()
    ...
    .catch(error => {
      logger.error(`POST /merle/contains {key: ${key}, hash: ${hash}} ${error.message || error}`);
      return errorResponse(response, error.message || error);
    })
})
...
router.get('/merkle/roots/:key', (request, response) => {
  ...
  KeyStoreHistory.findOne({key}).populate('history').exec()
    ...
    .catch(error => {
      logger.error(`GET /merle/roots/:key {key: ${key}} ${error.message || error}`);
      return errorResponse(response, error.message || error);
    })
})
...
router.get('/db/all', (request, response) => {
  KeyStoreHistory.find({}).populate('history').exec()
    ...
    .catch(error => {
      logger.error(`GET /db/all ${error.message || error}`);
      return errorResponse(response, error.message || error);
    })
})
...
router.delete('/db/all', (request, response) => {
  ...
  .catch(error => {
    logger.error(`DELETE /db/all ${error.message || error}`);
    return errorResponse(response, error.message || error);
  })
...
router.post('/proof/', checkBody, createLocalRecord, (request, response) => {
  const { key, store } = request.body;
  logger.debug(`POST /proof {key: ${key}, store: ${store}} keyStoreId: ${request.keyStoreId}`)
  proof.set(key, store, request.keyStoreId)
    .then(payload => {
      return successResponse(response, `CMD: set(${key}, ${store})`, {result: payload});
    })
    .catch(error => {
      logger.error(`POST /proof {key: ${key}, store: ${store}} ${error.message || error}`);
      return errorResponse(response, error.message || error);
    })
...
router.delete('/proof/', (request, response) => {
  ...
  proof.remove(key)
    ...
    .catch(error => {
      logger.error(`DELETE /proof {key: ${key}} ${error.message || error}`);
      return errorResponse(response, error.message || error);
    })
})
...
router.get('/proof/exists/:key', (request, response) => {
    ...
    proof.exists(key)
      ...
      .catch(error => {
        logger.error(`GET /proof/exists/:key {key: ${key}} ${error.message || error}`);
        return errorResponse(response, error.message || error);
      })
})
...
router.get('/proof/timestamp/:key', (request, response) => {
    ...
    proof.timestamp(key)
      ...
      .catch(error => {
        logger.error(`GET /proof/timestamp/:key {key: ${key}} ${error.message || error}`);
        return errorResponse(response, error.message || error);
      })
})
...
router.get('/balance/:address', (request, response) => {
  ...
  web3.web3Http.eth.getBalance(address)
    ...
    .catch(error => {
      logger.error(`GET /balance/:address {address: ${address}} ${error.message || error}`);
      return errorResponse(response, error.message || error);
    })
})
...
router.get('/transaction/:hash', (request, response) => {
  ...
  web3.web3Http.eth.getTransaction(hash)
    ...
    .catch(error => {
      logger.error(`GET /transaction/:hash {hash: ${hash}} ${error.message || error}`);
      return errorResponse(response, error.message || error);
    })
})
...
router.get('/transactionReceipt/:hash', (request, response) => {
  ...
  .catch(error => {
    logger.error(`GET /transactionReceipt/:hash {hash: ${hash}} ${error.message || error}`);
    return errorResponse(response, error.message || error);
  })
})
...
router.get('/pendingTransactions', (request, response) => {
  web3.web3Http.eth.txpool.content()
    ...
    .catch(error => {
      logger.error(`GET /pendingTransactions ${error.message || error}`);
      return errorResponse(response, error.message || error);
    })
})
...
router.get('/block/:blockNumber', (request, response) => {
  ...
  web3.web3Http.eth.getBlock(blockNumber)
    ...
    .catch(error => {
      logger.error(`GET /block/:blockNumber {blockNumber: ${blockNumber}} ${error.message || error}`);
      return errorResponse(response, error.message || error);
    })
})
...
router.get('/contract', (request, response) => {
  invoice.readContract('invoice')
    ...
    .catch(error => {
      logger.error(`GET /contract ${error.message || error}`);
      return errorResponse(response, error.message || error);
    })
})
...
router.post('/deploy/contract/proof', (request, response) => {
        ...
        Promise.all([
          ...
          web3.web3Http.eth.getBalance(process.env.WEB3_PUBLIC_KEY),
          web3.web3Http.eth.getTransactionCount(process.env.WEB3_PUBLIC_KEY, 'pending')
        ])
        .then(data => {
          ...
          web3.web3Http.eth.accounts.signTransaction(transactionParams, process.env.WEB3_PRIVATE_KEY)
        ...
        .catch(error => {
          logger.error(`POST /deploy/contract/proof ${error.message || error}`);
          return errorResponse(response, error.message || error);
        })
    })
